...
public class CommentaireControllerTest {
    ...
    void whenCreating_shouldReturnIsCreated_andShouldReturnURL() throws Exception {
        Long id = 78L;
        Commentaire toCreate = mock(Commentaire.class);
        when(toCreate.getId()).thenReturn(id);
    ...
    void whenUpdating_shouldReturnNoContent_andTheCorrectCommentaireShouldHaveBeenUsed() throws Exception {
        User u = new User(1L, "machin", Fonction.USER);
        Issue i = new Issue(2L, "titre", "contenu", u, Timestamp.from(Instant.now()), null);
        Commentaire toUpdate = new Commentaire(3L, u, i, "content");
        ArgumentCaptor<Commentaire> commentaire_captor = ArgumentCaptor.forClass(Commentaire.class);

        String obj = new ObjectMapper().writeValueAsString(toUpdate);
        ...
        Assertions.assertEquals(toUpdate.getAuthorId(), received.getAuthorId());
        Assertions.assertEquals(toUpdate.getIssueCode(), received.getIssueCode());
